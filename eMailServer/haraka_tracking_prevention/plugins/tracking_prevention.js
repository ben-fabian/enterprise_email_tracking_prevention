...
const MODE = process.env.MODE || "mta"; // set as 'mta' or 'proxy' or 'rebound'
const PROXY_MAIL_ADDRESS = process.env.PROXY_MAIL_ADDRESS || ""; // insert the proxy target address here and uncomment line 72
...
function escapedTrackedImgSources(trackedImages) {
  return trackedImages
    .map((trackedImage) => escapeRegExp(trackedImage.src))
    .join("|");
}
...
exports.hook_data_post = async (next, connection) => {
  var to =
    MODE === "proxy" ? PROXY_MAIL_ADDRESS : connection.transaction.rcpt_to;
  ...
  const trackedImages = await sendToDetectionEngine(
    mailExtractor(body, headers)
  );
  if (trackedImages.length) {
    ...
    body.bodytext = body.bodytext.replace(
      new RegExp(escapedTrackedImgSources(trackedImages), "ig"),
      "https://cdn.pixabay.com/photo/2016/07/07/08/14/stop-1502032_960_720.png"
    );
  }
